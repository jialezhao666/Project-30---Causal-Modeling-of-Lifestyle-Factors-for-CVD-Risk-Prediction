...
# Both baseline (22036-0.0) and imaging-visit (22036-2.0) now use the same self-report guideline-attainment instrument,
# ensuring definitional consistency in the longitudinal PA comparison.
...
# genetic_sex and age_defined_baseline 
...
def pa_official_baseline(col_22036_0):
    """
    Official imaging-visit PA indicator from field 22036-0.0 (baseline).
    """
    c = _clip_neg_na(col_22036_0)
...
pa_b  = pd.Series(pa_official_baseline(df['22036-0.0']), index=df.index)
...
INTERVENTIONS = ['smk', 'pa', 'sleep', 'pa_sleep', 'smk_pa']
INTERVENTION_LABELS = {
    'smk'     : 'Quit smoking',
    'pa'      : 'Increase PA',
...
_mask_specs = {
    ...
    'smk_pa': dict(
        # baseline unhealthy on BOTH; imaging improves BOTH (treated) or stays unhealthy on BOTH (control). Mixed changers excluded
        # Feasibility confirmed YES in 05_transition_matrix.py 
...
# How much of the main analysis's outcome-positive signal comes from participants whose CVD/AF/HF event occurred BEFORE their imaging visit
...
for name, coh in cohorts_sub.items():
    coh.to_parquet(os.path.join(OUTPUT_DIR, f'msm_cohort_{name}_sub.parquet'),
                index=False)
print(f"Saved 8 cohort parquet files (4 interventions x main/sub)")

# Steps 2-4 (IPTW weighting, bootstrap CI, E-value) are wrapped into run_pipeline() below and called once for cohorts_main and once for
# cohorts_sub, so both the main analysis and the sub-analysis excluding pre-imaging CVD events go through the identical pipeline.
...
def run_pipeline(cohorts, suffix, run_label):
    ...
    def bootstrap_ci(cohort_w, n_boot=N_BOOT, alpha=0.05):
        ...
        # Guard: if either arm has zero events, bootstrap is not meaningful.
        # Return NaN CIs rather than crashing on an empty rr_arr.
        if Y[idx_t].sum() == 0 or Y[idx_c].sum() == 0:
...
for name in INTERVENTIONS:
    ...
    print(f"{label:<16} {_f(rm['RD'])} {rm['treated_events']:>4} "
        f"{'Y' if rm['direction_consistent'] else 'N':>5}  | "
        f"{_f(rs['RD'])} {rs['treated_events']:>4} "
